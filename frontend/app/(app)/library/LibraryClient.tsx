    ...
    <div className="mx-auto max-w-6xl px-6 pb-20 lg:max-w-[1400px]">
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <div className="bg-[#7fb069] p-[3px] clip-input">
            <div className="relative bg-white px-6 py-4 clip-input">
              <Search size={18} className="absolute left-6 top-1/2 -translate-y-1/2 text-[#7fb069]" />
              <input
                type="text"
                placeholder="Search by title or author..."
                value={searchQuery}
                onChange={(event) => setSearchQuery(event.target.value)}
                className="w-full bg-transparent pl-10 text-sm font-semibold text-[#1a1a1a] outline-none"
              />
            </div>
          </div>
        </div>
      ...
      <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        {filtered.map((negotiation, index) => {
          ...
          return (
            <div
              key={negotiation.id}
              className={`bg-[#7fb069] p-[3px] transition hover:-translate-y-1 hover:shadow-xl ${clipClass}`}
            >
              <div className={`overflow-hidden bg-white ${clipClass}`}>
                ...
                <div className="space-y-3 p-6">
                  ...
                  <div className="text-lg font-serif text-[#1a1a1a]">{negotiation.title}</div>
                  <div className="text-xs text-[#666]">{negotiation.role}</div>
                  <div className="flex flex-wrap items-center gap-2 text-xs font-medium text-[#666]">
                  ...
                  <div className="flex items-center justify-between text-xs text-[#666]">
                    ...
                    <span className="rounded-full bg-[#7fb069]/10 px-3 py-1 text-xs font-semibold text-[#1a1a1a]">
